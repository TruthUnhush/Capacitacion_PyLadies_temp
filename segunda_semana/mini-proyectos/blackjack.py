...
while want_card:
    ...
    print(f"    Compurter's first card: {computer_hand[0]}")

    if sum(player_hand) > 21:
        want_card = False
        continue

    anonther_card = input("Type 'y' to get another card, type 'n' to pass:  ")
...
if sum(player_hand) < 22:
    ...
    while sum(computer_hand) < 21 and sum(computer_hand) < sum(player_hand):
        ...
        computer_hand.append(random.choice(data.cards))
        if computer_hand[-1] == 11 and sum(computer_hand) > 21:
            computer_hand.pop()
            computer_hand.append(1)
